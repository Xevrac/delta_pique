...
modified on 20190130 - yuyasato

modified on 20251123 - Xevrac

partial translation by Xevrac

All rights are reserved by yuyasato, Delta Networks holds no affiliation or representation to this person in any capacity.
"""
...
BOT_IN_BOTH     = True     #If true, join both teams and keep team balance
BOT_ADD_PATTERN = 1        #0: Always a fixed number (fixed count), 1: Always a fixed number (prioritize balance with slight increase/decrease) 2: Sum of humans and bots is always a fixed number, 3: A fixed number (of bots) per human player
BOT_ADD_NUM     = 24       #Value of the "fixed number"
LITE_MODE       = False    # Lightweight/Simplified crude calculation
LV_AUTO_ADJUST  = 0       # Level auto-adjustment mode 0:off 1:human vs bot 2:blue vs green 3:both human/bot and blue/green
BOT_NUM_NAME    = false    # Whether to use the ID number as the individual name
CPU_LV          = [16,12]   # BOT strength level: average value and plus/minus dispersion from that
BOTMUTE         = True    # bot chat off
DANSA_CALC_NUM  = 5        #How many steps (vertical changes) to calculate (for pathfinding)
KEIRO_TANSAKU_NUM = 5      #Number of route searches when avoiding a wall
ENEMY_SEARCH_FPS  = 4
BOT_UPDATE_RATE   = 1      #60/n FPS (integer)
LVCHANGE_RECONNECT= False  #Whether to reconnect when the level changes (if you want to include the level value in self.name)
LV_PRINT = False            #Print when the level changes

DEBUG_VIRTUAL_HIT=False # Virtual hit detection for debugging

AI_mode = 1 #Usage setting/Mode setting
...
if LITE_MODE:
    ...
    KEIRO_TANSAKU_NUM = 2
    ENEMY_SEARCH_FPS  = 4
    BOT_UPDATE_RATE   = 2      #60/n FPS (integer)
...
@command('addbot', admin_only = True)		#command example : /addbot 5 green
...
@command('toggleai', admin_only = True)	#Commands to stop and restore AI
...
def apply_script(protocol, connection, config):
    class BotProtocol(protocol):
            ...
            for bot in team.get_players():
                if bot.local and bot.world_object:
                    if bot.hp is None or bot.hp <= 0:
                        bot.disconnect() #If anyone is dead, eliminate them immediately.
                        return True
                    if bot.aim_at==None:
                        yobi = bot	#If there is anyone who is not in combat, eliminate them next.
            if yobi != None:
                yobi.disconnect() 
                return True
            return False
        ...
        def bot_num_adjust_calc(self):
            ...
            if BOT_ADD_PATTERN==0:#completely fixed
                if BOT_IN_BOTH:
                    blue = blue_human + blue_bot
                    green = green_human + green_bot
                    balance = blue-green
                    bot_over = blue_bot + green_bot - BOT_ADD_NUM
                    if bot_over == 0: #No problem with numbers
                        if balance >=2:#too much blue
                            return -1,1
                        elif balance <=-2:#too much green
                            return 1,-1
                        else:#No team balance issues, +-1 is acceptable
                            return 0,0
                    else:
                        if bot_over >1:#Too many bots
                            return -1,-1
        ...
        def on_grenade_thrown(self, grenade):
            ...
            for countt in range(int(3*UPDATE_FPS/dt)):
                ...
                if t>=fuse*UPDATE_FPS:
                    ...
                    ggreenata=[self, (x,y,z),exptime,mem]
                    self.protocol.exp_position.append(ggreenata)
                    callLater(fuse, self.removegrenadelist, ggreenata)
                    break
...
#					print "%.3f, %.3f, %.3f, (%.2f,%.2f,%.2f) "%(self.xoff_tebure, self.xoff_okure, self.large_omega_x,self.movezure_x,self.movezure_y,self.movezure_z)
        ...
        def gamewin(self):
            ...
            print("stage %s cleagreen"%self.protocol.stage_level)
